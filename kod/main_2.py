...
def single_point_crossover(parent1, parent2):
    """
    Tek nokta crossover fonksiyonu. İki ebeveynden rastgele bir noktadan çocuk oluşturur.
    Args:
        parent1 (list): Birinci ebeveyn (weights ve biases içerir).
        parent2 (list): İkinci ebeveyn (weights ve biases içerir).
    Returns:
        list: Yeni oluşturulan çocuk birey.
    """
    child_weights = []
    child_biases = []
    
    # Ağırlıklar için tek nokta crossover
    for w1, w2 in zip(parent1[0], parent2[0]):  # Katman bazında ağırlıklar
        w1 = np.array(w1)  # Listeleri NumPy dizilerine çeviriyoruz
        w2 = np.array(w2)  # Listeleri NumPy dizilerine çeviriyoruz
        crossover_point = random.randint(0, w1.shape[0] - 1)  # Rastgele bir kesim noktası
        child_weights.append(
            np.concatenate((w1[:crossover_point], w2[crossover_point:]), axis=0)  # axis=0, vektörlerin birleşmesi
        )
    
    # Biaslar için tek nokta crossover
    for b1, b2 in zip(parent1[1], parent2[1]):  # Katman bazında biaslar
        b1 = np.array(b1)  # Listeleri NumPy dizilerine çeviriyoruz
        b2 = np.array(b2)  # Listeleri NumPy dizilerine çeviriyoruz
        crossover_point = random.randint(0, len(b1) - 1)
        child_biases.append(
            np.concatenate((b1[:crossover_point], b2[crossover_point:]), axis=0)
        )
...
def elitist_roulette_selection(fitness_values, marios, num_elites=4, total_selected=16):
    ...
    Args:
        ...
        num_elites (int): Elit Mario sayısı (default: 4).
        total_selected (int): Seçilecek toplam Mario sayısı (default: 16).
    Returns:
        np.ndarray: Yeni jenerasyon Mario dizisi.
    """
    fitness_values = np.array(fitness_values)
    ...
    remaining_fitness = np.delete(fitness_values, elite_indices)
    remaining_marios = np.delete(marios, elite_indices, axis=0)
    
    # 3. Normalize fitness değerlerini olasılığa çevir
